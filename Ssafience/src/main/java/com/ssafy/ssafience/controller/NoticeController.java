...
public class NoticeController {
	...
	@PostMapping
	public ResponseEntity<BasicResponse> signup(@RequestBody NoticeWriteRequest request){
	...
	@PutMapping
	public ResponseEntity<BasicResponse> updateMember(@RequestBody NoticeModifyRequest request){
		final BasicResponse result = new BasicResponse();
		
		try {
			int updateNotice = nService.update(request);
			if (updateNotice == -1) {
				result.result = NOTAVAILABLE;
				result.status = HttpStatus.NO_CONTENT;
	...
	public ResponseEntity<BasicResponse> deleteMember(@PathVariable int noticeNo){
		final BasicResponse result = new BasicResponse();
		
		try {
			int deleteNotice = nService.delete(noticeNo);
			if (deleteNotice == -1) {
				result.result = NOTAVAILABLE;
				result.status = HttpStatus.NO_CONTENT;
				result.message = "없는 공지사항";
